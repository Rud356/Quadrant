...
class TestUserInDBCreation(unittest.TestCase):
    ...
    async def test_register_user_internally(self, session):
        user = await create_user(
            "Rud tester", login="Rud356",
            password="example_password_123A", session=session
        )
    ...
    async def test_register_duplicate_user_internally(self, session):
        with self.assertRaises(exc.IntegrityError):
            await create_user(
                "Rud tester", login="Rud356.2",
                password="example_password_123Ax", session=session
            )
            await create_user(
                "Rud tester 2", login="Rud356.2",
                password="example_password_123Ax", session=session
            )
